...
for b=1:numRows2
    Tc22(b,6)=unique(Tc22{b,6})
    Tc22{b,7}=unique(Tc22{b,7})

end
for b=1:numRows2
    PostASn=find(Tc22{b,7}==tabellaNumeriUser{:,1})
...
for j = 1:size(Nations,2)%dim nazioni poi
        ...
        idf2=find(ismember(Tc22Copy(:,6), Nations(j))) %trovo gli indici della nazione j
        Tc3=Tc2(idf,:)
        Tc4=Tc22Copy(idf2,:)
