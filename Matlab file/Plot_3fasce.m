...
for j = 1:Rows
    if(~cellfun('isempty',tabellaPesi(j,1)))
        ...
        ylim([0 200])
        title('Plot Median from 8:00 to 16:00')
        xlabel('2h Time Bins') 
        ylabel('Result(ms)') 
        legend(Nations{j})
        set(gcf,'color','w');
        fname = sprintf('%s_%s_Median_morning_%s', FileOut2{end},FileOut{2},Nations{j});
        ...
        ylim([0 200])
        title('Plot Median from 16:00 to 00:00')
        xlabel('2h Time Bins') 
        ylabel('Result(ms)') 
        legend(Nations{j})
        set(gcf,'color','w');
        fname = sprintf('%s_%s_Median_afternoon_%s', FileOut2{end},FileOut{2},Nations{j});
        export_fig(['/home/guazzelli/disco/Thesis/Matlab file/plot/' fname], '-pdf');
        set(gcf, 'Visible', 'off');
        figure('Visible', 'off')
        scatter(tabsera.DateCorrectFormat,tabsera.y,'x');
        ylim([0 200])
        title('Plot Median from 00:00 to 8:00')
        xlabel('2h Time Bins') 
        ylabel('Result(ms)') 
        legend(Nations{j})
        set(gcf,'color','w');
        fname = sprintf('%s_%s_Median_night_%s', FileOut2{end},FileOut{2},Nations{j});
        export_fig(['/home/guazzelli/disco/Thesis/Matlab file/plot/' fname], '-pdf');
