function[] = AFR_ALL_TESTSign(param1,param2)

AllCountryAlpha2=readtable('paesiAlpha2')
AfricaALpha2=readtable('AfricaALpha2')

[Rows,Cols] = size(AfricaALpha2)
for p=1:Rows
    tappo=find(AllCountryAlpha2{:,3}==string(AfricaALpha2{p,2}))
    valore = cellstr(AllCountryAlpha2{tappo,2});
    AfricaALpha2{p,3}=valore(1,1);
end
...
africaNations=AfricaALpha2{:,3}.'
africaNationscell=cellstr(africaNations)
IndiciRegion=find(ismember(T{:,4},africaNationscell))
TAfrica=T(IndiciRegion,:)
G = findgroups(TAfrica{:,5});     
Tc = splitapply( @(varargin) varargin, TAfrica, G);
...
Tc2AFR=Tc(idxd,:) % isolo solo quelli con più di 1o probe
Tc=[]


% Next find population served by the ASs
ASNnumUSERNationsonly=readtable('Asn_people_nations_new.csv')
IndiciRegionASN=find(ismember(ASNnumUSERNationsonly{:,3},africaNationscell))
ASNnumUSERNationsonly2=ASNnumUSERNationsonly(IndiciRegionASN,:)
tabellaNumeriUserAFR = groupsummary(ASNnumUSERNationsonly2,'ASN','sum','Users_est__');
...
TierTable_UniqueSum= groupsummary(TierTable,'Var1','sum','Var2');
 ...
 % single as name for each row
Tc2AFR(:,5)=num2cell(cellfun(@unique,Tc2AFR(:,5)))
[numRows2,numCols2] = size(Tc2AFR)
for b=1:numRows2
    Tc2AFR{b,4}=unique(Tc2AFR{b,4})
end


for b=1:numRows2
    PostASn=find(Tc2AFR{b,5}==tabellaNumeriUserAFR{:,1})
    Tc2AFR{b,6}=tabellaNumeriUserAFR{PostASn,3}
end
...
IndiciRegion=find(ismember(T{:,4},africaNationscell))
TAfrica2020=T(IndiciRegion,:)
G = findgroups(TAfrica2020{:,5});     
Tc = splitapply( @(varargin) varargin, TAfrica2020, G);
asn_id=cellfun(@unique,Tc(:,1),'UniformOutput',false) % asn unici
[cellsz1, cellsz2] = cellfun(@size,asn_id,'UniformOutput',false); %dimensione di ciascun gruppo
conv=cell2mat(cellsz1)
idxd=find(conv>=2) %trovo le posizioni dei magg di 10
Tc2AFR2020=Tc(idxd,:) % isolo solo quelli con più di 1o probe
Tc=[]
...
tabellaNumeriUserAFR = groupsummary(ASNnumUSERNationsonly2,'ASN','sum','Users_est__');
 ...
 % single as name for each row
Tc2AFR2020(:,5)=num2cell(cellfun(@unique,Tc2AFR2020(:,5)))
[numRows2,numCols2] = size(Tc2AFR2020)
for b=1:numRows2
    Tc2AFR2020{b,4}=unique(Tc2AFR2020{b,4})
end

for b=1:numRows2
    PostASn=find(Tc2AFR2020{b,5}==tabellaNumeriUserAFR{:,1})
    Tc2AFR2020{b,6}=tabellaNumeriUserAFR{PostASn,3}
end


T=[]
Tc2AFR2020Copy=Tc2AFR2020
%solo i primi 6 mesi del 2020
for v=1:size(Tc2AFR2020,1)
    primi6=find(Tc2AFR2020{v,2}<='2020-06-30 23:55:55')
    Tc2AFR2020{v,2}=Tc2AFR2020{v,2}(primi6,1)
    Tc2AFR2020{v,1}=Tc2AFR2020{v,1}(primi6,1)
    Tc2AFR2020{v,3}=Tc2AFR2020{v,3}(primi6,1)
end
...
ylim([0 2100])
hold on
title('Africa 90 Percentile')
xlabel('4h Time Bins') 
ylabel('Result(ms)') 
set(gcf,'color','w');
fname ='AFR_16430285_90Perc2020'
...
ylim([0 2100])
title('Plot 90 Percentile morning')
xlabel('4h Time Bins') 
ylabel('Result(ms)') 

set(gcf,'color','w');
fname = sprintf('%s_%s_90Perc_morning', FileOut2,FileOut{2});
export_fig(['C:/Users/guazz/Desktop/' fname], '-pdf');
set(gcf, 'Visible', 'off');
figure('Visible', 'off')
scatter(tabpomeriggio.DateCorrectFormat,tabpomeriggio.y,'+');
ylim([0 2100])
title('Plot 90 Percentile afternoon')
xlabel('4h Time Bins') 
ylabel('Result(ms)') 

set(gcf,'color','w');
fname = sprintf('%s_%s_90Perc_afternoon', FileOut2,FileOut{2});
export_fig(['C:/Users/guazz/Desktop/' fname], '-pdf');
set(gcf, 'Visible', 'off');
figure('Visible', 'off')
scatter(tabsera.DateCorrectFormat,tabsera.y,'x');
ylim([0 2100])
title('Plot 90 Percentile night')
xlabel('4h Time Bins') 
ylabel('Result(ms)') 

set(gcf,'color','w');
fname = sprintf('%s_%s_90Perc_night', FileOut2,FileOut{2});
export_fig(['C:/Users/guazz/Desktop/' fname], '-pdf');
