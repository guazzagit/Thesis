...
s_train = pd.read_csv(Input, index_col=0, parse_dates=True,usecols=[1,2,6])#ricorda di rimett123
...
for elem in lista:
	...
	TimeBins = rslt_df.groupby(pd.Grouper(level='timestamp',freq='240min'))["resultset.result.rt"].apply(lambda x: prctile(x,90,)).reset_index() # numero totale di timebins di 2h per dati senza errori
	#print(TimeBins)240
	...
	outlier_detector = OutlierDetector(LocalOutlierFactor(n_neighbors=1,p=1,contamination=0.05))
